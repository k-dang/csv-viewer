...
- **Record empty state.** Run `node .agents/skills/verify-csv-viewer/bin/control-csv-viewer.mjs snapshot --path evidence/open-csv/empty.aria.txt` and `screenshot --path evidence/open-csv/empty.png`. Both show `CSV Viewer`, `No CSV open`, and the seeded Recent files.
