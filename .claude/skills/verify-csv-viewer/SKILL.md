...
- **web** is the browser build. CSV Sources are `File` objects held for the page's lifetime, opened through an `<input type="file">`. There are no Recent CSV Sources and no native dialogs.
...
```powershell
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs <command>
```
...
```powershell
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs launch
```

For the web target, add `--web`:

```powershell
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs launch --web
```

Rebuild first after source changes (either target):

```powershell
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs launch --rebuild
```
...
1. Runs `pnpm run build:web` instead, producing `apps/web/dist-web/`.
2. Starts `bin/web-browser/`, an Electron shim with **no preload**, which serves `dist-web` over `http://127.0.0.1:<webPort>` from its own process and opens one window at it. `window.csvViewer` never exists, so the page runs its real web composition.
3. Points downloads at `runs/<id>/downloads/` so Export CSV lands on disk without a dialog.
...
The web port is isolated per run and is never Vite's 5173, so a dev server the user already has open cannot be reached or clobbered.

The web target runs on Electron's bundled Chromium. It proves the web build's behavior, not cross-browser support. A Firefox or Safari claim needs a human.
...
```powershell
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs doctor
```
...
- `userDataDir` is under `.claude/skills/verify-csv-viewer/runs/`
...
```powershell
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs click --role button --name "phase-2-sample.csv"
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs click --role button --name "Close stats panel" --nth 0
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs fill --role searchbox --name "Global search" --value "Ada"
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs fill --focused --value "Ada Lovelace Edited"
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs type --text "Ada"
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs wait --text "1 visible of 5 rows" --timeout 10000
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs press --key Enter
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs snapshot --path evidence/open-csv/after.aria.txt
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs screenshot --path evidence/open-csv/after.png
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs text
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs upload --role button --name "Open CSV" --nth 0 --file fixtures/phase-2-sample.csv
```

Relative `--path` values resolve under `.claude/skills/verify-csv-viewer/`.
...
On **web** there are no native dialogs. `upload --file` answers the file input, so a second CSV, the comparison feature, and the Export CSV round trip are all provable unattended. Exported bytes land in `runs/<id>/downloads/` (`doctor` prints `downloadDir`); read them to prove the export really contains the edit.
...
Put artifacts under `.claude/skills/verify-csv-viewer/evidence/<feature-id>/`. Cleanup must not delete this directory.
...
```powershell
node .claude/skills/verify-csv-viewer/bin/control-csv-viewer.mjs cleanup
```

Cleanup kills the pid from `current.json` (process tree, not the name `electron`), deletes that run directory (userData, logs, and on web the static server and its `downloads/`), and deletes `current.json`. It leaves `.claude/skills/verify-csv-viewer/evidence/` in place.
...
| Command | Purpose |
| --- | --- |
| `launch [--web] [--rebuild]` | Build if needed, start an isolated instance of the chosen target, wait until healthy. Desktop seeds Recent CSV Sources; web serves `dist-web` and captures downloads |
...
Two `pnpm run dev` processes cannot share port 5173, and this skill does not use Vite. The web target serves its own build on an isolated port instead, so it never collides with a dev server the user has open. Two built processes can run if they have different `--user-data-dir` and CDP ports, but the helper keeps one recorded run, and desktop and web share that one slot. If `current.json` is live, launch exits: `cleanup` before switching targets. Never drive a window this run did not start.
