...
Cleanup kills the pid from `current.json` (process tree, not the name `electron`), deletes that run directory (userData, logs), and deletes `current.json`. It leaves `.claude/skills/verify-csv-viewer/evidence/` in place.
