...
Cleanup kills the pids from `current.json` (process trees, not process names): the app or browser, plus the dev server on web. It then deletes that run directory (profile, logs, and on web `downloads/`) and `current.json`. It leaves `.claude/skills/verify-csv-viewer/evidence/` in place.
