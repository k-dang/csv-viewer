...
- Native Open/Save dialogs are out of band. Open CSVs from Recent files. Do not click `Open CSV` or `Save CSV as` in unattended runs.

## Driving conventions

- Start every recipe from the launched empty window unless the feature file says otherwise.
- Prefer `--role` plus `--name` over CSS or coordinates.
- Treat `Compare…` as the ellipsis character `…`, not three dots.
- After each mutation, wait for concrete text (`Ready`, row counts, `Unsaved changes`, comparison badges).
...
- Edits are proven by grid text plus `Unsaved changes`. Re-read the fixture to prove the source was not overwritten.
- Report an unreachable path with the command and the unmet precondition. Do not report a skipped native dialog as verified through Recent files.
...
- [Edit a CSV](./edit-csv.md) covers cell edits, insert, append, delete, undo/redo, and the undriveable Save As dialog.
