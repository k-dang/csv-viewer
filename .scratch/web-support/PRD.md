...
Refactor CSV Viewer around one injected `CsvViewer` module and one shared, runtime-neutral CsvWorkspace implementation. React depends only on domain operations and explicit capabilities. Electron, web, and tests provide adapters at the CsvViewer seam, while native DuckDB and DuckDB-Wasm provide adapters at a smaller internal database seam.
...
Web support is complete only when the same domain behavior available on desktop - Working CSV querying, sorting, filtering, search, statistics, editing, undo/redo, Export CSV, and Aligned Comparison - passes against DuckDB-Wasm. Runtime differences are limited to explicit capabilities with current readers, such as Recent CSV Source availability, export delivery, and capacity. Cancellation latency is not a product capability or domain guarantee.
...
- **One shared product architecture:** Desktop and web share the renderer, `CsvViewer` interface, and runtime-neutral `CsvWorkspace` instead of maintaining separate workspaces or an independently evolving web edition. This prevents domain behavior from drifting and keeps Electron and browser mechanics behind narrow host and database adapters. Genuine runtime differences are represented as explicit capabilities rather than platform checks, optional no-ops, or duplicated product rules.
...
Two properties of the single-threaded asynchronous DuckDB-Wasm build are load-bearing and conflict with how desktop Aligned Comparison works today. The ticket 00 spike verified them against both the newest stable build and the newest available same-core build; detailed evidence is recorded in `spike-findings.md`.

- **Connection separation is logical, not parallel.** The desktop `ComparisonExecutor` requests a dedicated worker connection (`connectWorker`) alongside the owner connection. On the single-threaded Wasm build every connection multiplexes onto one Worker thread, so two connections give isolation of transaction and artifact ownership, not simultaneous DuckDB execution. The API used to issue long work is load-bearing: blocking `query()` starved an owner query, while the pending-query `send()` API yielded Worker scheduling turns and let the owner query complete before one long comparison statement. Comparison work therefore uses the pending-query path; bounded SQL statements are an optional measured optimization rather than an architectural requirement. Connection separation remains a real invariant for ownership and cleanup.
- **Cancellation has two adapter shapes but one observable contract.** Desktop cancels through `connection.interrupt()`. DuckDB-Wasm has no method with that shape, but `AsyncDuckDBConnection.cancelSent()` cancels a statement already executing through `send()`; in the spike it cancelled a multi-second comparison CTAS, published no table, and left the connection usable. Cancellation is not limited to application-created statement boundaries. The database adapter must pair Wasm `send()` with `cancelSent()` and normalize it with native interruption. Observable contract behavior remains: a cancelled operation reports cancelled, publishes nothing, and releases its artifacts. Cancellation latency is not a domain guarantee.
...
- Use one `CsvViewer` interface rather than separate workspace and renderer operation interfaces. Declare each operation once through the typed request map, carry requests through one `call` method, publish events through one subscription, and expose only capabilities with current readers.
- Inject `CsvViewer` at the renderer composition root. React must not access the Electron preload global, IPC channel names, browser File objects, file handles, download elements, Workers, or runtime-name checks.
...
- Instantiate the shared CsvWorkspace in the Electron main process on desktop and in the browser page on web. Desktop provides a `CsvViewer` IPC proxy over the main-process workspace; web provides the same interface through direct in-page wiring. Every operation is asynchronous, and every request, result, and subscription event is structured-clone-safe. The interface contains no live objects, callbacks inside results, or synchronous accessors.
...
- Keep native DuckDB on desktop; do not migrate Electron to DuckDB-Wasm in this scope. The wasm32 address-space ceiling is the only hard architectural difference between the engines - both already run in memory - and it sits above the file sizes CSV Viewer targets, so it does not force the choice either way. Migrating would buy real deletion (the native package and its per-platform rebuild, the native adapter, engine version-parity work, and dual-engine contract execution), but it would trade multi-threaded native execution for single-threaded Worker execution on no evidence, and it does not gate the web release. Retain both adapters. Revisit only if native packaging or version parity becomes the dominant maintenance cost, and treat CI cost from dual-engine test execution as a test-scheduling problem instead.
- Use DuckDB-Wasm's single-threaded asynchronous Worker build. Do not require cross-origin isolation or adopt experimental multithreading in the first release. Accept serialized execution across connections, and issue long cancellable work through the pending-query API so Worker scheduling and `cancelSent()` remain available.
...
- The primary product and domain seam is `CsvViewer`. Use it to test shared behavior and capability-dependent presentation without Electron or browser globals. Run one extensive behavioral contract against a native-DuckDB-backed CsvViewer and a DuckDB-Wasm-backed CsvViewer.
...
- Assert cancellation by observable outcome, not by latency: a cancelled operation reports cancelled, publishes no result, and releases its artifacts. Adapter tests prove that native `interrupt()` and Wasm `send()` with `cancelSent()` produce that outcome. Shared contract tests do not depend on either driver mechanism or an elapsed-time threshold.
...
Ticket 04 established one injected `CsvViewer` interface for the renderer. The later simplification concentrated product operations in one typed request map, one `call` method, and one event subscription. Desktop transport carries those requests and events mechanically; web calls the same interface in the page. No separate renderer runtime interface should be reintroduced.
