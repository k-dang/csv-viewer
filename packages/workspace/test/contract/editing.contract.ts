...
export function defineCsvWorkspaceEditingContract(factory: WorkspaceContractFactory): void {
  describe(`${factory.name} CsvWorkspace editing, history, and Export CSV contract`, () => {
    ...
    it('keeps a later row query valid after inserting a column the query does not name', async () => {
      const workingCsv = await openPeople('insert-under-query.csv');
      const request = { workingCsvId: workingCsv.workingCsvId };
      const query = {
        sort: [{ column: 'name', direction: 'asc' as const }],
        filters: [{ column: 'name', kind: 'text' as const, operator: 'contains' as const, value: 'a' }],
        search: 'a',
      };

      const before = await readRows(workingCsv.workingCsvId, query);
      await workspace().call({
        operation: 'csv.insert-column',
        ...request,
        column: 'team',
      ...
      const after = await readRows(workingCsv.workingCsvId, query);

      expect(rowIds(before.rows)).toEqual(['1', '2']);
      expect(rowIds(after.rows)).toEqual(['1', '2']);
      expectVisibleRows(after.rows).toEqual([
        { name: 'Ada', team: 'compiler', 'New column': '' },
        { name: 'Grace', team: 'navy', 'New column': '' },
      ]);
    });

    it('keeps a later row query valid after deleting a column the query does not name', async () => {
      const workingCsv = await openPeople('delete-unmentioned.csv');
      const request = { workingCsvId: workingCsv.workingCsvId };
      const query = {
        sort: [{ column: 'name', direction: 'asc' as const }],
        filters: [{ column: 'name', kind: 'text' as const, operator: 'contains' as const, value: 'a' }],
      ...
      expect(deleted.columns.map((column) => column.name)).toEqual(['name']);
      expect(rowIds(before.rows)).toEqual(['1', '2']);
      expect(rowIds(after.rows)).toEqual(['1', '2']);
      expectVisibleRows(after.rows).toEqual([{ name: 'Ada' }, { name: 'Grace' }]);
    });

    it('rejects a sort on a deleted column and returns source order without that sort', async () => {
      const workingCsv = await openPeople('delete-sorted.csv');
      const request = { workingCsvId: workingCsv.workingCsvId };
      const sort = [{ column: 'name', direction: 'desc' as const }];

      await workspace().call({ operation: 'csv.delete-column', ...request, column: 'name' });

      await expect(readRows(workingCsv.workingCsvId, { sort })).rejects.toThrow('Unknown CSV column: name');
      const source = await readRows(workingCsv.workingCsvId);
      expect(rowIds(source.rows)).toEqual(['1', '2', '3']);
      expectVisibleRows(source.rows).toEqual([{ team: 'compiler' }, { team: 'navy' }, { team: 'kernel' }]);
    });

    it('rejects a filter on a deleted column and returns the rows that filter had hidden', async () => {
      const workingCsv = await openPeople('delete-filtered.csv');
      const request = { workingCsvId: workingCsv.workingCsvId };
      const filters = [{ column: 'team', kind: 'text' as const, operator: 'equals' as const, value: 'compiler' }];

      const filtered = await readRows(workingCsv.workingCsvId, { filters });
      expectVisibleRows(filtered.rows).toEqual([{ name: 'Ada', team: 'compiler' }]);

      await workspace().call({ operation: 'csv.delete-column', ...request, column: 'team' });

      await expect(readRows(workingCsv.workingCsvId, { filters })).rejects.toThrow('Unknown CSV column: team');
      const restored = await readRows(workingCsv.workingCsvId);
      expectVisibleRows(restored.rows).toEqual([{ name: 'Ada' }, { name: 'Grace' }, { name: 'Linus' }]);
    });
