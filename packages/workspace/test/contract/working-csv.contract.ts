...
export function defineCsvWorkspaceWorkingCsvContract(
  ...
  describe(`${factory.name} CsvWorkspace Working CSV contract`, () => {
    ...
    it('opens a CSV Source and returns its description, inferred columns, and row count', async () => {
      ...
      expect(workingCsv.columns.map((column) => column.name)).toEqual([
        'name',
        'age',
        'joined',
      ]);
      expect(workingCsv.columns.map((column) => column.type)).toEqual([
        'VARCHAR',
        'VARCHAR',
        'VARCHAR',
      ]);
    ...
    it('handles quoted fields, escaped quotes, and embedded delimiters', async () => {
      const workingCsv = await fixture.openSource(
        ...
        [
          'name,note',
          'Ada,"uses commas, quotes ""well"", and new lines"',
          'Grace,"plain"',
        ].join('\n'),
      );
    ...
    it('reopens a Working CSV behind its stable identity with a new revision', async () => {
      const auto = await fixture.openSource(
        'reopen.txt',
      ...
      if (outcome.status !== 'opened')
        throw new Error(`Reopen was ${outcome.status}.`);
      ...
      expect(reopened.columns.map((column) => column.name)).toEqual([
        'name',
        'age',
      ]);
    });
    ...
    it('increments revisions and notifies dependents for every committed data change', async () => {
      ...
      const reopened = await workspace().call({
        operation: 'csv.reopen',
        workingCsvId: workingCsv.workingCsvId,
      });
      if (reopened.status !== 'opened')
        throw new Error(`Reopen was ${reopened.status}.`);
    ...
    it('keeps edit journals independent per Working CSV', async () => {
      ...
      await expect(
        fixture.editState(first.workingCsvId),
      ).resolves.toMatchObject({
        hasUnexportedChanges: true,
      });
      await expect(
        fixture.editState(second.workingCsvId),
      ).resolves.toMatchObject({
    ...
    it('reopens one Working CSV without disturbing another', async () => {
      ...
      expectVisibleRows(reopenedRows.rows).toEqual([
        { name: 'Ada', score: '10' },
      ]);
      expectVisibleRows(secondRows.rows).toEqual([
        { name: 'Grace', score: '20' },
      ]);
    });
    ...
    it('sorts rows by a structured column descriptor and clears sort by omitting descriptors', async () => {
      ...
      expect(originalOrder.rows.map((row) => row.name)).toEqual([
        'Ada',
        'Grace',
        'Linus',
      ]);
    ...
    it('keeps hidden row identifiers stable across sorting, filtering, search, and pagination', async () => {
      ...
      const firstPage = await workspace().call({
        operation: 'csv.get-rows',
        ...request,
        limit: 2,
      });
      ...
      expect(workingCsv.columns.map((column) => column.name)).not.toContain(
    ...
    it('applies text, numeric, and combined filters with filtered row counts', async () => {
      ...
      expectVisibleRows(window.rows).toEqual([
        { name: 'Margaret', age: '87', team: 'compiler' },
      ]);
    });
        ...
        values: [
          {
            value: 'Closed',
            count: 2,
            percentOfScope: expect.closeTo(33.333, 3),
          },
          {
            value: 'Open',
            count: 2,
          ...
          },
          {
            value: 'Pending',
            count: 1,
            percentOfScope: expect.closeTo(16.667, 3),
          },
          {
            value: 'open',
            count: 1,
    ...
    it('applies Count Scope from filters and search while ignoring sort order', async () => {
      ...
      expect(sortedRows.rows.map((row) => row.name)).toEqual([
        'Barbara',
        'Margaret',
        'Ada',
      ]);
    ...
    it('composes search with active filters using parameterized search values', async () => {
      ...
      expect(filteredSearch.rows.map((row) => row['full name'])).toEqual([
        'Margaret Hamilton',
      ]);
