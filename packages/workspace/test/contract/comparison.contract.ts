...
async function refresh(
  ...
  if (completed.status !== 'applied')
    throw new Error(`Refresh completed as ${completed.status}.`);
  return completed.comparison;
}

async function comparisonState(
  value: WorkspaceContractFixture,
  comparisonId: string,
): Promise<ComparisonView> {
  const comparison = value.latestComparison(comparisonId);
...
export function defineCsvWorkspaceComparisonContract(
  ...
  describe(`${factory.name} CsvWorkspace Comparison contract`, () => {
    ...
    it.each([
      ...
      'summarizes $name from literal expected outcomes',
      async ({ baseline, candidate, expected }) => {
    ...
    it('aligns a composite key with exact cells, all classifications, binary ordering, and a final partial window', async () => {
      ...
      const applied = await applyKey(value, comparison.comparisonId, [
        'group',
        'id',
      ]);
      ...
      const swapped = await value.viewer.call({
      ...
      expect(swapped.comparison.baseline.workingCsvId).toBe(
        candidate.workingCsvId,
      );
    ...
    it('returns complete invalid-key counts with bounded non-overlapping evidence', async () => {
      ...
      expect(completed.diagnostics.candidate).toEqual({
        ...
        duplicateExamples: [
          { keyValues: ['A', '2'], rowCount: 2, rowIds: ['2', '3'] },
        ],
      });
      expect(
        (await comparisonState(value, comparison.comparisonId)).applied,
      ).toBeNull();
    });
    ...
    it('keeps the applied result readable when a replacement key is invalid', async () => {
      ...
      const applied = await applyKey(value, comparison.comparisonId, ['id']);
      if (!applied.applied)
        throw new Error('Comparison has no applied result.');
      ...
      const invalid = await runComparison(value, comparison.comparisonId, [
        'id',
      ]);
      ...
      await expect(
        readWindow(value, invalid.comparison),
    ...
    it('reports compatibility and key-shape faults through typed workspace outcomes', async () => {
      ...
      await expect(
        ...
        status: 'rejected',
        fault: { code: 'incompatible-columns' },
      });
      await expect(
        value.viewer.call({
          operation: 'comparison.open',
    ...
    it('cancels before publication without publishing a result', async () => {
      ...
      await expect(
        value.awaitComparisonOutcome(started.operationId),
      ).resolves.toEqual({
    ...
    it('handles key-only data, zero and maximum windows, and Swap sides without recomputation', async () => {
      ...
      expect(
        ...
        status: 'rejected',
        fault: { code: 'invalid-window' },
      });
      const maximum = await readWindow(value, applied, {
        limit: 1_000,
        rows: 'differences',
      });
      ...
      expect(
        (
          await readWindow(value, swapped.comparison, { rows: 'differences' })
        ).rows.map(observableRow),
      ).toEqual([
        {
          classification: 'candidate-only',
          keyValues: ['2'],
    ...
    it('closes dependent Comparisons and releases their published results with a CSV Source', async () => {
      ...
      const confirmation = await value.viewer.call({
        operation: 'csv.close',
        workingCsvId: baseline.workingCsvId,
      });
      if (confirmation.status !== 'confirmation-required') {
        throw new Error(
          `CSV Source close completed as ${confirmation.status}.`,
        );
      }
    ...
    it('marks every Working CSV data mutation Outdated while queries and Export CSV remain current', async () => {
      ...
      await value.viewer.call({
        ...
        filters: [
          { column: 'value', kind: 'text', operator: 'contains', value: 'old' },
        ],
        search: 'old',
      });
      await value.viewer.call({
      ...
      await value.writeSource(
        'baseline.csv',
        ['id,value', '1,replaced', ''].join('\n'),
      );
      const replacement = await value.viewer.call({
