...
**Working CSV**:
The CSV data of one CSV Tab, including cell edits, inserted rows, deleted rows, and inserted, deleted, and renamed columns. Each CSV Tab has its own Working CSV, independent of other Tabs. Column Value Counts are calculated from the Working CSV, not from its CSV Source.
