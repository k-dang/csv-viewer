...
CSV Viewer is built for practical CSV work: open files from disk, browse large datasets smoothly, adjust parsing options, search and filter rows, make focused cleanup edits, then deliver the Working CSV separately with Export CSV.
...
Recent CSV Sources are stored in Electron's per-user `userData` directory as `recent-files.json`. CSV Sources are never modified.
